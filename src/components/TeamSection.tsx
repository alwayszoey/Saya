...
function DiscordWidget() {
  ...
  useEffect(() => {
    ...
    async function fetchDiscord() {
      try {
        ...
        if (!res.ok) throw new Error('Discord fetch error');
        const json = await res.json();
        const guildId = json.guild?.id ?? json.guild_id ?? '';
        const iconHash = json.guild?.icon ?? json.profile?.icon_hash;
        const iconUrl =
          guildId && iconHash
        ...
        if (active) {
          setDiscordData({
            name: json.guild?.name ?? json.profile?.name ?? 'Zyni',
            onlineCount: json.approximate_presence_count ?? json.profile?.online_count ?? 380,
            memberCount: json.approximate_member_count ?? json.profile?.member_count ?? 13000,
            iconUrl,
            inviteUrl: defaultDiscordInvite
          });
        }
      } catch {
        if (active) {
  ...
  const inviteUrl = discordData?.inviteUrl ?? defaultDiscordInvite;
  const guildName = discordData?.name ?? 'Zyni';
  const onlineFormatted = discordData ? formatNumber(discordData.onlineCount) : '—';
  const membersFormatted = discordData ? formatNumber(discordData.memberCount) : '—';

  return (
    <div className="team-widget team-widget--discord">
      <div className="team-widget-discord-top">
        <div
          className={`team-widget-discord-icon${discordData?.iconUrl ? ' team-widget-discord-icon--live' : ''}`}
          aria-hidden={discordData?.iconUrl ? undefined : true}
        >
          {discordData?.iconUrl ? (
            <img src={discordData.iconUrl} alt="" className="team-widget-discord-icon-img" />
          ) : (
      ...
      <div className="team-widget-discord-join">
        <span>เข้าร่วมพูดคุยกับเรา</span>
        <a
          href={inviteUrl}
          target="_blank"
          rel="noreferrer"
